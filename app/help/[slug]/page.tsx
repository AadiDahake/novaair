  ...
  return (
    <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_290px] lg:items-start">
      <article>
        ...
        <p className="mt-6 text-sm font-bold uppercase tracking-[0.14em] text-blue-soft">
          {article.category}
        </p>
        <h1 className="mt-2 max-w-2xl text-[2.6rem] font-extrabold leading-[1.08] tracking-tight text-ink">
          {article.title}
        </h1>
              ...
              <ul className="mt-3 max-w-2xl space-y-2.5">
                {section.body.map((line) => (
                  <li key={line} className="flex gap-3 text-[0.95rem] leading-relaxed text-ink-soft">
                    <span aria-hidden="true" className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-blue-soft" />
                    <span>{line}</span>
                  </li>
                ))}
              </ul>
      ...
      <aside className="space-y-6">
              ...
              {related.map((entry) => (
                <li key={entry.slug}>
                  <Link
                    ...
                    className="text-sm font-medium text-ink-soft underline transition-colors hover:text-ink"
                  >
                    {entry.title}
                  </Link>
                </li>
        ...
        <section aria-labelledby="contact-heading" className="rounded-[20px] bg-blue-tint p-6">
          <h2 id="contact-heading" className="text-base font-bold text-ink">
            Still need help?
          </h2>
          <p className="mt-2 text-sm leading-relaxed text-ink-soft">
            Call NovaAir on 1-800-555-0142, every day from 05:00 to 23:00 Pacific Time.
          </p>
          <Link href="/my-booking" className="pill pill-light mt-5 px-5 py-2.5 text-[0.85rem]">
            Open My Booking
          </Link>
        </section>
      </aside>
